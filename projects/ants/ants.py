...
class Ant(Insect):
    ...
    def double(self):
        ...
        while place.exit:
            place = place.exit
            if place.ant:
                if not place.ant.is_double:
                    place.ant.is_double = True
                    place.ant.damage = 2 * place.ant.damage
                if place.ant.is_container and place.ant.ant_contained:
                    if not place.ant.ant_contained.is_double:
                        place.ant.ant_contained.is_double = True
                        place.ant.ant_contained.damage = 2 * place.ant.ant_contained.damage
        # END Problem 12
    ...
    def nearest_bee(self):
        ...
        while True:
            if place.is_hive:
                ...
                if bound >= self.lower_bound and bound <= self.upper_bound:
    ...
    def insects_in_front(self):
        ...
        while not place.is_hive:
            if place.ant is not None:
                if place.ant.is_container and place.ant.ant_contained.name != 'Laser':
                    insects[place.ant.ant_contained] = distance
                elif place.ant.name != 'Laser':
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if hasattr(self, 'scared') and self.scared:
            if self.scared_length > 0:
                self.scared_length -= 1
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        if not (hasattr(self, 'scared') and self.scared):
            self.scared = True
            self.scared_length = length
        # END Problem EC 2
