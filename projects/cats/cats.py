...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    new = [n for n in paragraphs if select(n)]
    if k < len(new):
        return new[k]
    else:
        return ''
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def f(paragraph):
        for x in split(lower(remove_punctuation(paragraph))):
            for y in subject:
                if x == y:
                    return True
        return False
...
def accuracy(typed, source):
    ...
    for i in range(len(source_words)):
        if i < len(typed_words) and source_words[i] == typed_words[i]:
            count += 1
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    typed_list = [item for item in typed]
    return len(typed_list) / 5 / (elapsed / 60)
    # END PROBLEM 4
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        "*** YOUR CODE HERE ***"
        immutable_keys = (typed, source)
        if immutable_keys not in cache:
            result = diff_function(typed, source, limit)
            cache[immutable_keys] = [limit, result]
            return result
        else:
            cache_limit = cache[immutable_keys][0]
            if limit <= cache_limit:
                return cache[immutable_keys][1]
            else:
                result = diff_function(typed, source, limit)
                cache[immutable_keys] = [limit, result]
                return result
...
def furry_fixes(typed, source, limit):
    ...
    if typed == source:
        ...
        add_count = 0 if typed[0] == source[0] else 1
        return add_count + furry_fixes(typed[1:], source[1:], limit - add_count)
    # END PROBLEM 6
...
def final_diff(typed, source, limit):
    ...
    if typed[0] == source[0]:
        return minimum_mewtations(typed[1:], source[1:], limit)
    elif typed[0:2] == source[0:2][::-1]:
        return 1 + minimum_mewtations(typed[2:], source[2:], limit - 1)
    elif source[0:2] == source[0:2][::-1]:
        return 1 + minimum_mewtations(typed[:], source[2:], limit - 1)
    else:
        add = 1 + minimum_mewtations(typed[:], source[1:], limit - 1)
        remove = 1 + minimum_mewtations(typed[1:], source[:], limit - 1)
        substitute = 1 + minimum_mewtations(typed[1:], source[1:], limit - 1)
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(source)):
        if i < len(typed) and typed[i] == source[i]:
            count += 1
        else:
...
def time_per_word(words, timestamps_per_player):
    ...
    for time in tpp:
        ...
        for i in range(len(time) - 1):
            new_time.append(time[i + 1] - time[i])
...
def fastest_words(words_and_times):
    ...
    for word_index in word_indices:
        fastest_num = 0
        fastest_time = get_time(times, fastest_num, word_index)
        for player_num in player_indices:
            time = get_time(times, player_num, word_index)
            if time < fastest_time:
                fastest_time = time
                fastest_num = player_num
        result[fastest_num].append(words[word_index])
    return result
    # END PROBLEM 10
