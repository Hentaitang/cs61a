...
def scheme_eval(expr, env, tail=False):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        operator = scheme_eval(first, env, tail)
        operands = rest.map(lambda x: scheme_eval(x, env, tail))
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        if expressions.rest is nil:
            ...
            res = scheme_eval(expressions.first, env, False)
...
def optimize_tail_calls(unoptimized_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        "*** YOUR CODE HERE ***"
        while True:
            val = unoptimized_scheme_eval(result.expr, result.env, tail)
            if not isinstance(val, Unevaluated):
                return val
            result = val
