...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        if len(rest) == 1:
            env.bindings[signature] = scheme_eval(rest.first, env)
        else:
            env.bindings[signature] = scheme_eval(rest, env)
        return signature
        # END PROBLEM 4
...
def do_if_form(expressions, env):
    ...
    if is_scheme_true(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.rest.first, env, True if len(expressions) == 2 else False)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        formal = bindings.first
        validate_form(formal, 2, 2)
        names = Pair(formal.first, names)
        vals = Pair(scheme_eval(formal.rest.first, env), vals)
